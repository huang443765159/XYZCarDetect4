...
class CarDetect:
    ...
    def set_dis_list(self, dis_list: list):  # 边缘出发
        ...
        if self._was_stopped != was_stopped:
            ...
            self.sign.car_stopped.emit(self._was_stopped, cur_frame)
            if self._network.tcp.is_connected():
                tx_msg = encode(head=CODEC.HEAD_TO_NUC, was_stopped=was_stopped,
                                frame=cur_frame, timestamp=get_timestamp())
                self._network.tcp.send(tx_msg)
